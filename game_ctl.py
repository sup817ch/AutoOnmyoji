...
class GameControl():
    ...
    def wait_game_color(self,region,color,tolerance=0,max_time=30,quit=True):
        """
        等待游戏颜色
    ...
    def quit_game(self):
        ...
        退出游戏
        """
        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))) #记录时间
        self.window_full_shot('tmp\\full.png') #保存一下现场
